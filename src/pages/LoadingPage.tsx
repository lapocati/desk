...
const LoadingPage = () => {
  ...
  const beginInitialDialoguePrefetch = useAppStore((state) => state.beginInitialDialoguePrefetch);
  const resolveInitialDialoguePrefetch = useAppStore((state) => state.resolveInitialDialoguePrefetch);
  const rejectInitialDialoguePrefetch = useAppStore((state) => state.rejectInitialDialoguePrefetch);
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        const nextSoulPool = { ...useAppStore.getState().hiddenSoulPool, ...analysis.spiritScores };
        ...
        void getDialogueResponse(1, [], analysis.hiddenDoubts, nextSoulPool)
          .then((response) => {
            if (cancelled) return;
          ...
          .catch((err) => {
            if (cancelled) return;
            rejectInitialDialoguePrefetch(
              requestId,
              err instanceof Error ? err.message : '灵宠走神了，请稍后重试'
            );
          });
    ...
    setLoading,
    updateSoulPool,
    beginInitialDialoguePrefetch,
    resolveInitialDialoguePrefetch,
    rejectInitialDialoguePrefetch,
    resetInitialDialoguePrefetch,
  ]);
