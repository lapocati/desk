...
必须为【dialogue_spirits】中的每个灵宠各生成 1 条吐槽，条数与列表一致（1~3 条）。speaker 必须是该灵宠中文名（智慧灵/活力灵/治愈灵/奇想灵/守护灵之一），一一对应、不重复。
...
function formatDialogueSpirits(history: DialogueMessage[]): string {
  const spirits = extractDialogueSpirits(history);
  if (spirits.length === 0) {
    return '1. 智慧灵';
  }
  return spirits.map((s, i) => `${i + 1}. ${SPIRIT_LABELS[s]}`).join('\n');
}

function getExpectedSpeakerLabels(history: DialogueMessage[]): string[] {
  const spirits = extractDialogueSpirits(history);
  if (spirits.length === 0) {
    return [SPIRIT_LABELS.wisdom];
  }
  return spirits.map((s) => SPIRIT_LABELS[s]);
}
...
export async function getObservationAndResult(
  ...
  const expectedSpeakers = getExpectedSpeakerLabels(dialogueHistory);
...
【dialogue_spirits】
${formatDialogueSpirits(dialogueHistory)}
    ...
    try {
      ...
      if (attempt < MAX_RESULT_TEXT_RETRIES) {
        retryMessages.push({
          role: 'user',
          content:
            '上次输出格式有误。请重新生成，必须严格输出包含 discoveries（1-5条）、spiritComments（每条含 speaker+quote，条数与 dialogue_spirits 一致）、resonanceZones（4-6个）、scientificAdvice（3-4条）、spiritAdvice（1-2条）、dynamicPersonality（≤25字）、resonanceReasons（恰好3条且每条≤18字）的纯 JSON 对象。',
        });
        continue;
      }
  ...
  try {
    ...
    const retryMessages = [
      ...messages,
      {
        role: 'user' as const,
        content:
          '上次输出格式有误。请重新生成，必须严格输出包含 discoveries（1-5条）、spiritComments（条数与 dialogue_spirits 一致）、resonanceZones（4-6个）、scientificAdvice（3-4条）、spiritAdvice（1-2条）的纯 JSON 对象。',
      },
    ];
