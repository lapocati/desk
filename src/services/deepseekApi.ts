...
const OBSERVATION_SYSTEM_PROMPT = `你是"桌灵档案馆"的观察记录生成助手。你将在每一轮探索大循环结束时，为用户生成一份守护灵视角的《灵居观察记录》。你必须以温柔、好奇、绝不说教的陪伴者语气，将桌面痕迹、用户回答与灵宠世界观编织成一份有启发性的轻报告。

**一、输出格式（严格 JSON，不加任何额外文本）**
{
  "evidenceChain": "string（四段式证据链）",
...
1. evidenceChain（证据链推理）
   - 必须按四段结构输出，中间用" | "分隔：
     [桌面物理现象] → [用户三轮回答关键词] → [推理逻辑] → [心境/习惯映射]
   - 物理现象使用视觉报告中的物品及位置描述。
   - 用户回答关键词从三轮对话中提取。
   - 推理逻辑要清晰、共情，绝不能武断诊断，必须用"可能""似乎""看起来"等推测词。
   - 心境映射要落回五大灵宠关注的心理维度（专注力、行动力、情绪、创造力、安全感）。

2. spiritObservation（灵居气息观察）
   - 以灵宠口吻进行东方文化趣味解读，严禁出现"风水"、"风水学"、"命理"、"算命"等词汇。
   - 可使用"从灵居气息来看"、"这里的能量流动"、"桌面小宇宙"等替代表达。
   - 语气保持轻松、陪伴，用"或许可以留意一下"、"也许它在提醒你"这类柔和建议。

3. scientificAdvice（科学收纳建议，占70%）
   - 必须输出 3–4 条具体、可执行、基于效率与空间整理的物理调整方案。
   - 每条建议必须包含明确的方位、物品类型及行动指令。
   - 建议需要关联当前桌面现象，不能泛泛而谈。

4. spiritAdvice（灵居气息建议，占30%）
   - 输出 1–2 条顺应灵宠世界观的环境微调建议。
   - 建议依旧要落到具体物品或空间调整，但用守护灵的口吻包装。
   - 避免玄学化，强调这是一种轻松的自我关照仪式。

**三、全局语气与禁忌**
- 全程使用"我注意到……"、"或许可以试试……"、"如果愿意的话……"等推测与邀请句式。
- 绝不对用户进行评判，哪怕用户桌面极度混乱，也要用正向框架描述。
- 禁止出现任何与医学、心理健康诊断相关的词汇（如"抑郁症"、"焦虑症"）。
- 禁止提及具体分数、百分比或用户未表达的情绪。
- 所有分析必须建立在"守护灵的好奇与陪伴"上，而非专家审视。`;
...
function parseObservationResponse(rawText: string): ObservationRecord {
  ...
  const evidenceChain = String(parsed.evidenceChain || '').trim();
  const spiritObservation = String(parsed.spiritObservation || '').trim();
  ...
  if (!evidenceChain || !spiritObservation) {
...
${formatTop3(soulPool)}`;
  ...
  try {
    ...
    return parseObservationResponse(rawText);
  } catch (firstError) {
    const retryMessages = [
      ...messages,
      {
        role: 'user' as const,
        content:
          '上次输出格式有误。请重新生成，必须严格输出包含 evidenceChain、spiritObservation、scientificAdvice（3-4条）、spiritAdvice（1-2条）的纯 JSON 对象。',
      },
    ];
...
function parseResultTextsResponse(rawText: string): {
  ...
  try {
    ...
    const jsonMatch = rawText.match(/\{[\s\S]*\}/);
    if (!jsonMatch) {
      throw new Error('结果页文案返回格式异常，无法解析 JSON');
    }
    parsed = JSON.parse(jsonMatch[0]);
  }
  ...
  if (!dynamicPersonality) {
    throw new Error('结果页文案缺少趣味副人格');
  }
  if (dynamicPersonality.length > 25) {
    throw new Error('趣味副人格超过 25 字');
  }
  if (resonanceReasons.length !== 3) {
    throw new Error('共鸣原因必须为 3 条');
  }
  for (const reason of resonanceReasons) {
    if (reason.length > 18) {
      throw new Error('共鸣原因单条超过 18 字');
    }
  }
...
【灵居气息观察】
${observationRecord?.spiritObservation || '（观察记录生成中，请结合视觉与对话推断）'}
...
${base.personalityTag}`;
  ...
  try {
    const rawText = await requestDeepSeekResult(messages);
    const texts = parseResultTextsResponse(rawText);
    return { ...base, ...texts };
  } catch (firstError) {
    const retryMessages = [
      ...messages,
      {
        role: 'user' as const,
        content:
          '上次输出不符合约束。请重新生成：dynamicPersonality ≤25字，resonanceReasons 恰好3条且每条 ≤18字，以"· "开头，严格 JSON。',
      },
    ];
    const rawText = await requestDeepSeekResult(retryMessages);
    try {
      const texts = parseResultTextsResponse(rawText);
      return { ...base, ...texts };
    } catch {
      throw firstError instanceof Error ? firstError : new Error('结果页文案生成失败');
    }
  }
}
